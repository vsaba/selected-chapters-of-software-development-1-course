...
/**
 * A class which represents the tokens situated inside a For - tag.
 * The class has 4 different possible elements of the {@link Element} class.
 * It is obligated to contain at least one {@link ElementVariable}
 * @author Vito Sabalic
 *
 */
public class ForLoopNode extends Node{
	...
	/**
	 * A simple constructor which assigns all four provided values to the each current value respectively
	...
	/**
	 * A simple constructor which assigns three provided values to each curretn value respectively
	 * @param variable the {@link ElementVariable} to be assigned
	 * @param start the first expression to be assigned
	 * @param end the last expression to be assigned
	 */
	...
	/**
	 * Returns the string value of the implemented values.
	 * Takes into account the Child nodes of the current class
	 */
	@Override
	public String toString() {
		String s = "{$FOR ";
		if(stepExpression != null) {
			 s = s + getVariable().asText() + getStartExpression().asText() + getEndExpression().asText() + getStepExpression().asText() + "$}";
		}
		else {
